    ...
    internal static partial class ModImportService
    {
        ...
        private static ModImportResult ImportInternal(IReadOnlyList<string> modDirectories, ClusterCollection vanillaClusterData, string mergeDisplayName, bool includeTopology, int clusterHexGap)
        {
            ...
            foreach (string modDirectory in modDirectories)
            {
                ...
                if (ImportFileDiscoveryHelper.EnumerateMapXmlFiles(modDirectory).Count == 0)
                {
                    throw new DirectoryNotFoundException($"The selected extension does not contain any importable map XML files: {modDirectory}");
                }
            }
            ...
            foreach (string modDirectory in modDirectories)
            {
                foreach (string xmlPath in ImportFileDiscoveryHelper.EnumerateMapXmlFiles(modDirectory))
                {
            ...
            var customClusterPositions = galaxyConnections
                ...
                .ToDictionary(
                    ...
                    a =>
                    {
                        ConnectionDefinition first = a.First();
                        return new ImportedClusterPlacement(first.Offset, ExtractConnectionName(first.Path));
                    },
        ...
        private static List<Cluster> BuildImportedClusters(
            ...
            Dictionary<string, ImportedClusterPlacement> customClusterPositions,
            ReferencedVanillaEndpoints referencedVanillaEndpoints)
        {
            ...
            var clusterMacroNames = new List<string>();
            var seenClusterMacroNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var macro in macros.Values.Where(a => a.IsCluster))
            {
                AddClusterMacroName(macro.Name, clusterMacroNames, seenClusterMacroNames);
            }

            foreach (var macroName in customClusterPositions.Keys)
            {
                AddClusterMacroName(macroName, clusterMacroNames, seenClusterMacroNames);
            }
            ...
            foreach (var clusterMacroName in clusterMacroNames)
            {
                try
                {
                    ...
                    var cluster = isBaseCluster
                        ...
                        {
                            ImportedMacroName = clusterMacroName,
                            ImportedConnectionName = customClusterPositions.GetValueOrDefault(clusterMacroName)?.ConnectionName,
                            ...
                            Position = ConvertClusterPosition(customClusterPositions.GetValueOrDefault(clusterMacroName)?.Offset),
                            Sectors = []
                        };
                    ...
                    if (clusterMacro != null)
                    {
                        ...
                        {
                            var sector = CreateImportedSector(sectorConnection.MacroRef, sectorConnection.Offset, cluster, macros, vanillaLookup, referencedVanillaEndpoints, preserveOriginWhenOffsetMissing: true);
                            if (sector != null)
                            {
                    ...
                    if (isBaseCluster)
                    {
                        foreach (var sectorMacroName in touchedBaseSectors)
                        {
                            ...
                            var sector = CreateImportedSector(sectorMacroName, null, cluster, macros, vanillaLookup, referencedVanillaEndpoints, preserveOriginWhenOffsetMissing: false);
                            if (sector != null)
                            {
                                cluster.Sectors.Add(sector);
                            }
        ...
        private static Sector CreateImportedSector(
            ...
            VanillaLookup vanillaLookup,
            ReferencedVanillaEndpoints referencedVanillaEndpoints,
            bool preserveOriginWhenOffsetMissing)
        {
            try
            {
                ...
                Point? customOffset = ImportedSectorOffsetHelper.ResolveCustomOffset(
                    isBaseSector,
                    preserveOriginWhenOffsetMissing,
                    sectorOffset == null ? null : new Point((int)Math.Round(sectorOffset.X), (int)Math.Round(sectorOffset.Z)));
        ...
        private static void AddClusterMacroName(string macroName, List<string> orderedNames, HashSet<string> seenNames)
        {
            if (string.IsNullOrWhiteSpace(macroName) || !seenNames.Add(macroName))
                return;

            orderedNames.Add(macroName);
        }
        ...
        {
            List<string> godPaths = ImportFileDiscoveryHelper.EnumerateGodXmlFiles(modDirectory);
            if (godPaths.Count == 0)
                return;
            Dictionary<Sector, Dictionary<string, int>> sectorVotes = new();

            foreach (string godPath in godPaths)
            {
                XDocument document = XDocument.Load(godPath);

                foreach (GodStationPlacement placement in GodStationImportHelper.ParsePlacements(document))
                {
                    foreach (Sector sector in ResolveSectorsForGodLocation(importedClusters, placement.LocationClass, placement.LocationMacro))
                    {
                        Zone zone = GodStationImportHelper.EnsureZoneForPlacement(sector, placement.LocationClass, placement.LocationMacro);
                        AddGodStationIfMissing(sector, zone, placement);
                    }
                }
        ...
        private static void AddGodStationIfMissing(Sector sector, Zone zone, GodStationPlacement placement)
        {
            string stationName = placement.StationId;
            if (string.IsNullOrWhiteSpace(stationName))
                return;

            bool alreadyExists = sector.Zones
                .SelectMany(a => a.Stations)
                .Any(a => string.Equals(a.Name, stationName, StringComparison.OrdinalIgnoreCase));
            ...
            Station station = new()
            {
                ...
                Name = stationName,
                Faction = GodGeneration.CorrectFactionName(placement.Faction?.ToLowerInvariant()),
                Owner = NormalizeSectorOwner(placement.Owner),
                Race = placement.Race?.ToLowerInvariant(),
                Type = placement.Type ?? "factory",
                Position = placement.Position ?? zone.Position,
                LocationType = placement.LocationClass,
                Location = placement.LocationMacro,
                CustomConstructionPlan = placement.StationMacro == null ? placement.ConstructionPlan : null,
                ImportedGodStationId = placement.StationId,
                ImportedGodType = placement.RawType,
                ImportedGodStationMacro = placement.StationMacro,
                ImportedGodConstructionPlan = placement.ConstructionPlan,
                ImportedGodQuotaGalaxy = placement.QuotaGalaxy,
                ImportedGodQuotaSector = placement.QuotaSector,
                ImportedGodQuotaZone = placement.QuotaZone,
                ImportedGodQuotaGamestart = placement.QuotaGamestart,
                ImportedGodSolitary = placement.Solitary,
                ImportedGodLocationMatchExtension = placement.MatchExtension,
                ImportedGodLoadoutLevel = placement.LoadoutLevel,
                ImportedGodLoadoutVariation = placement.LoadoutVariation
            };
        ...
        private static string BuildZonePath(Cluster cluster, Sector sector, Zone zone)
        {
            ...
            string clusterConnectionName = GetImportedOrGeneratedClusterConnectionName(cluster);
            ...
            return $"{clusterConnectionName}/PREFIX_SE_c{cluster.Id:D3}_s{sector.Id:D3}_connection/PREFIX_ZO_c{cluster.Id:D3}_s{sector.Id:D3}_z{zone.Id:D3}_connection";
        }

        private static string GetImportedOrGeneratedClusterConnectionName(Cluster cluster)
        {
            if (!string.IsNullOrWhiteSpace(cluster.ImportedConnectionName))
                return cluster.ImportedConnectionName;

            return $"PREFIX_CL_c{cluster.Id:D3}_connection";
        }
        ...
        private static TranslationLookup LoadTranslations(IEnumerable<string> modDirectories)
        {
            ...
            foreach (string modDirectory in modDirectories)
            {
                foreach (string file in ImportFileDiscoveryHelper.EnumerateTranslationXmlFiles(modDirectory))
                {
        ...
        private sealed record ImportedClusterPlacement(PositionDefinition Offset, string ConnectionName);
