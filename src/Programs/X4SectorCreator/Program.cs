...
namespace X4SectorCreator
{
    internal static class Program
    {
        ...
        private static bool IsHelpRequested(string[] rawArgs)
        {
            return rawArgs.Any(a =>
        ...
        private static string BuildHelpText()
        {
            ...
            builder.AppendLine("X4SectorCreator command line usage");
            builder.AppendLine();
            builder.AppendLine("Usage:");
            builder.AppendLine("  X4SectorCreator [options]");
            builder.AppendLine();
            builder.AppendLine("Import options:");
            builder.AppendLine("  --import-mod <path>                Import a single X4 extension folder");
            builder.AppendLine("  --import-mod-merge <path>          Import all discovered extension folders under a root");
            builder.AppendLine("  --merge-import <path>              Alias for --import-mod-merge");
            builder.AppendLine("  --fix-sector-islands [path]        Import and repair isolated sectors");
            builder.AppendLine("  --fix-islands [path]               Alias for --fix-sector-islands");
            builder.AppendLine("  --fix-reverse-paths                Normalize reverse gate paths during import");
            builder.AppendLine("  --export-import-json <path>        Write the imported in-memory model to JSON after import");
            builder.AppendLine("  --export-import-xml <path>         Write the imported in-memory model to XML after import");
            builder.AppendLine("  --cluster-hex-gap <n>              Reserve a hex gap around imported clusters");
            builder.AppendLine("  --log-scanned-files                Write included and skipped file logs during import");
            builder.AppendLine("  --open-galaxy-view                 Open galaxy view after startup import");
            builder.AppendLine("  --exit-after-import                Close automatically after startup import completes");
            builder.AppendLine();
            builder.AppendLine("Audit options:");
            builder.AppendLine("  --check-sector-islands [path]      Audit unresolved sector islands");
            builder.AppendLine("  --check-import [path]              Audit imported cluster and sector totals");
            builder.AppendLine("  --check-name-resolution [path]     Audit sector name resolution");
            builder.AppendLine("  --check-included-files [path]      Audit included versus skipped files");
            builder.AppendLine("  --list-sector-names [path]         Print imported sector names");
            builder.AppendLine();
            builder.AppendLine("Other options:");
            builder.AppendLine("  --fix-unified-sector-translation [path]  Repair unified sector translation pages");
            builder.AppendLine("  --log-file <path>                  Write session diagnostics to a specific file");
            builder.AppendLine("  --wine-path                        Convert provided Unix-style paths to Wine paths");
            builder.AppendLine("  --proton-path                      Alias for --wine-path");
            builder.AppendLine("  --custom-galaxy                    Enable custom galaxy output");
            builder.AppendLine("  --galaxy-name <name>               Set the custom galaxy name");
            builder.AppendLine("  --disable-all-storylines           Disable all storylines for generated output");
            builder.AppendLine("  --help                             Show this help text");
            builder.AppendLine("  -h                                 Show this help text");
            builder.AppendLine("  /?                                 Show this help text");
    ...
    public sealed class StartupOptions
    {
        ...
        public string ExportImportJsonPath { get; private set; }
        public string ExportImportXmlPath { get; private set; }
        public bool LogScannedFilesOnImport { get; private set; }
        ...
        public static StartupOptions Parse(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                switch (arg.ToLowerInvariant())
                {
                    ...
                    case "--fix-gate-reverse-paths":
                        ...
                        break;
                    case "--export-import-json":
                    case "--export-import-result-json":
                    case "--dump-import-json":
                        if (i + 1 >= args.Length)
                            throw new ArgumentException("Missing path after --export-import-json");
                        options.ExportImportJsonPath = args[++i];
                        break;
                    case "--export-import-xml":
                    case "--export-import-result-xml":
                    case "--dump-import-xml":
                        if (i + 1 >= args.Length)
                            throw new ArgumentException("Missing path after --export-import-xml");
                        options.ExportImportXmlPath = args[++i];
                        break;
                    case "--log-scanned-files":
                    case "--log-included-files":
                    case "--log-skipped-files":
                        options.LogScannedFilesOnImport = true;
                        break;
            ...
            if (options.ConvertPathsToWineStyle)
            {
                ...
                options.ExportImportJsonPath = ConvertToWinePath(options.ExportImportJsonPath);
                options.ExportImportXmlPath = ConvertToWinePath(options.ExportImportXmlPath);
