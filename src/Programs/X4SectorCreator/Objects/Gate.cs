...
namespace X4SectorCreator.Objects
{
    public class Gate : ICloneable
    {
        ...
        private static string ConvertToPath(string modPrefix, Cluster cluster, Sector sector, Zone zone, Gate gate)
        {
            ...
            {
                string clusterConnection = !string.IsNullOrWhiteSpace(cluster.ImportedConnectionName)
                    ? cluster.ImportedConnectionName.Replace("PREFIX", modPrefix)
                    : $"{modPrefix}_CL_c{cluster.Id:D3}_connection";
