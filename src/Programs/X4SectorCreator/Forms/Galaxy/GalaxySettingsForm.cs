    ...
    public partial class GalaxySettingsForm : Form
    {
        ...
        private static void RemoveBaseGameGateConnections(List<(Cluster Cluster, Sector Sector, Gate Gate)> pairs)
        {
            foreach ((Cluster Cluster, Sector Sector, Gate Gate) pair in pairs)
            {
                ...
                if (!ImportedGateSectorResolver.TryFindCounterpartReference(MainForm.Instance.AllClusters.Values, pair.Gate, out ImportedGateSectorResolver.GateReference sourceReference))
                    continue;

                _ = sourceReference.Zone.Gates.Remove(sourceReference.Gate);

                // Check to remove zone if empty
                if (sourceReference.Zone.Gates.Count == 0)
                {
                    _ = sourceReference.Sector.Zones.Remove(sourceReference.Zone);
                }
            }
        }
        ...
        {
            return ImportedGateSectorResolver.TryFindCounterpartReference(MainForm.Instance.AllClusters.Values, gate, out ImportedGateSectorResolver.GateReference sourceReference)
                ? sourceReference.Cluster
                : null;
        }
