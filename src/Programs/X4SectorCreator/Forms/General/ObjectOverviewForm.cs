    ...
    public partial class ObjectOverviewForm : Form
    {
        ...
        class DataObject
        {
            ...
            public DataObject(Gate gate)
            {
                ...
                if (!ImportedGateSectorResolver.TryFindCounterpartReference(MainForm.Instance.AllClusters.Values, gate, out ImportedGateSectorResolver.GateReference sourceReference))
                {
                    throw new InvalidOperationException($"Unable to resolve reverse connection for gate '{gate.SourcePath ?? gate.ParentSectorName}'.");
                }

                Name = $"{gate.ParentSectorName} -> {sourceReference.Gate.ParentSectorName}";
